...
#region original
    ...
    def getProfiles(self, profile_id: str = '') -> pd.DataFrame:
        """Retrieves your list of profiles"""

        # GET /profiles
        if len(profile_id) == 0:
            df = self.authAPI('GET', 'profiles')
        else:
            df = self.authAPI('GET', f'profiles/{profile_id}')
        if len(df) == 0:
            return pd.DataFrame()
    ...
    def getOrderID(self, id: str = '', market: str = '', action: str = '', status: str = 'all') -> pd.DataFrame:
        """Retrieves your list of orders with optional filtering"""
        #region validate and filter
        # if market provided
        if market != '':
        ...
        df['fill_fees'] = df['fill_fees'].astype(float).round(8)
        df['filled_size'] = df['filled_size'].astype(float).round(8)
        df['executed_value'] = df['executed_value'].astype(float).round(8)
        if 'funds' in df:
            df['funds'] = df['funds'].astype(float).round(8)
            df['size'] = None
        else:
            df['size'] = df['size'].astype(float).round(8)
            df['funds'] = None
        # if id != '' and df['side'][0] == 'sell':
        #     df['sold'] = id
        # else:
        df['sold'] = '0'
    ...
    def getOrders(self, market: str = '', action: str = '', status: str = 'all') -> pd.DataFrame:
        ...
        if status != 'open':
            df['price'] = df.apply(lambda row: (float(row.executed_value) * 100) / (
...
class PublicAPI(AuthAPIBase):
    ...
    def getprice(self, market:str) -> float:
